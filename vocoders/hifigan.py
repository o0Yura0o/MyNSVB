...
class HifiGAN(PWG):
    def __init__(self):
        ...
        if os.path.exists(config_path):
            ckpt_paths = glob.glob(os.path.join(base_dir, 'model_ckpt_steps_*.ckpt'))
            if not ckpt_paths:
                raise FileNotFoundError(f'No HiFi-GAN ckpt found in {base_dir}')
            ckpt = sorted(
                ckpt_paths,
                key=lambda p: int(re.findall(r'model_ckpt_steps_(\d+)\.ckpt', os.path.basename(p))[0])
            )[-1]
