...
hparams = {}
_CACHE_PATH = os.path.join(tempfile.gettempdir(), "nsvb_hparams_cache.json")
...
def dump_hparams_cache():
    """Dump merged hparams to a temp JSON file for child processes to reuse."""
    try:
        # hparams 是 dict-like，可直接序列化（若有不可序列化項，這裡可再做過濾）
...
def load_hparams_cache():
    """Load hparams from the temp JSON if current global hparams is empty."""
    try:
        # 確認尚未初始化
        if (not hparams) and os.path.exists(_CACHE_PATH):
            with open(_CACHE_PATH, "r", encoding="utf-8") as f:
                data = json.load(f)
            # hparams 是全域 dict-like，直接更新即可
