...
from utils.hparams import hparams, load_hparams_cache
from utils.pitch_utils import denorm_f0

def _hp(key, default=None):
    # 確保在 Windows spawn 的子程序也有 hparams
    if not hparams:
        load_hparams_cache()
    try:
        return hparams.get(key) if hasattr(hparams, "get") else getattr(hparams, key)
    except Exception:
        return default
...
class SVBParaTask(FastSpeech2AdvTask):
    def __init__(self):
        ...
        self.concurrent_ways = _hp('concurrent_ways').split(',')
        self.train_ds = self.dataset_cls('train')

    def build_tts_model(self):
        data_dir = _hp('binary_data_dir')
    ...
    def run_model(self, model, sample, tech_prefix=None, return_output=False, infer=False):
        ...
        # spk_ids = sample['spk_ids'] if _hp('use_spk_id') else None
    ...
    def gen_cheat_disc(self, way, model_out_with_gradient, log_outputs, loss_weights):
        self.disc_cond_gt[way] = self.model_out[way]['decoder_inp'].detach() if _hp('use_cond_disc') else None
        self.disc_cond[way] = disc_cond_current_way = self.model_out[way]['decoder_inp'].detach() \
            if _hp('use_cond_disc') else None
        ...
        if p_ is not None:
            ...
            loss_weights[f'{way}_a'] = _hp('lambda_mel_adv')
        if pc_ is not None:
            log_outputs[f'{way}_ac'] = self.mse_loss_fn(pc_, pc_.new_ones(pc_.size()))
            loss_weights[f'{way}_ac'] = _hp('lambda_mel_adv')
    ...
    def _training_step(self, sample, batch_idx, optimizer_idx):
        log_outputs = {}
        loss_weights = {}
        disc_start = _hp('mel_gan') and self.global_step > _hp('disc_start_steps') and \
                     _hp('lambda_mel_adv') > 0
        if optimizer_idx == 0:
            #######################
            #      Generator      #
            #######################
            ...
            #    Discriminator    #
            #######################
            if disc_start and self.global_step % _hp('disc_interval') == 0:
                for way in self.concurrent_ways:
    ...
    def validation_step(self, sample, batch_idx):
        ...
        if self.global_step % _hp('valid_infer_interval') == 0 \
                and batch_idx < _hp('num_valid_plots'):
            if self.vocoder is None:
                self.vocoder = get_vocoder_cls(hparams)()
            sampling_rate = _hp('audio_sample_rate')
    ...
    def after_infer(self, predictions, sil_start_frame=0):
        ...
        gen_dir = os.path.join(_hp('work_dir'),
                               f'generated_{self.trainer.global_step}_{_hp("gen_dir_name")}')
    ...
    def save_result(wavs_dict, base_fn, gen_dir, str_phs=None, mels_dict=None):
        for key in wavs_dict:
            if _hp('disable_map'):
                ...
                audio.save_wav(wavs_dict[key], f'{gen_dir}/wavs/{key}/{base_fn}.wav', _hp('audio_sample_rate'),
                               norm=_hp('out_wav_norm'))
...
class ParaPPGPretrainedTask(SVBParaTask):
    def build_model(self):
        ...
        utils.load_ckpt(self.model.vc_asr, _hp('pretrain_asr_ckpt'), model_name='model')
        ...
        if _hp('load_ckpt') != '':
            self.load_ckpt(_hp('load_ckpt'), strict=False)
    ...
    def configure_optimizers(self):
        ...
        optimizer_gen = torch.optim.AdamW(
            self.gen_params,
            lr=_hp('lr'),
            betas=(_hp('optimizer_adam_beta1'), _hp('optimizer_adam_beta2')),
            weight_decay=_hp('weight_decay'))
        optimizer_disc = torch.optim.AdamW(
            self.disc_params,
            lr=_hp('disc_lr'),
            betas=(_hp('optimizer_adam_beta1'), _hp('optimizer_adam_beta2')),
            **_hp('discriminator_optimizer_params')) if len(self.disc_params) > 0 else None
    ...
    def validation_step(self, sample, batch_idx):
        ...
        outputs = utils.tensors_to_scalars(outputs)
        if self.global_step % _hp('valid_infer_interval') == 0 \
                and batch_idx > 394 and batch_idx < 394 + _hp('num_valid_plots'):
            if self.vocoder is None:
                self.vocoder = get_vocoder_cls(hparams)()
            sampling_rate = _hp('audio_sample_rate')
...
class ParaPPGSpkConsistentTask(ParaPPGPretrainedTask):
    ...
    def build_disc_model(self):
        ...
        if _hp('mel_gan'):
            disc_win_num = _hp('disc_win_num')
            h = _hp('mel_disc_hidden_size')
            self.spk_disc = Discriminator(
                time_lengths=[32, 64, 128][:disc_win_num],
                freq_length=80, hidden_size=h, kernel=(3, 3),
                cond_size=_hp('hidden_size') if self.use_cond_disc else 0,
                norm_type=_hp('disc_norm'), reduction=_hp('disc_reduction')
            )
    ...
    def gen_cheat_disc(self, way, model_out_with_gradient, log_outputs, loss_weights):
        ...
        if p_ is not None:
            ...
            loss_weights[f'{way}_spk'] = _hp('lambda_mel_adv')
        if pc_ is not None:
            log_outputs[f'{way}_spkc'] = self.mse_loss_fn(pc_, pc_.new_ones(pc_.size()))
            loss_weights[f'{way}_spkc'] = _hp('lambda_mel_adv')
